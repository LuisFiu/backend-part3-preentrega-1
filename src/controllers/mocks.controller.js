...
import {
  generateMockPets,
  generateMockUsers,
} from "../services/mocksServices.js";
...
const generateData = async (req, res) => {
  try {
    ...
    if (!users || !pets) {
      return res
        .status(400)
        .json({ status: "error", messag: "Incomplete values" });
    }

    if (typeof users == !"number" && typeof pets == !"number") {
      return res
        .status(400)
        .json({ status: "error", messag: "Invalid values format" });
    }
    ...
    return res
      .status(201)
      .json({ status: "success", payload: { generatedUsers, generatedPets } });
  } catch (error) {
